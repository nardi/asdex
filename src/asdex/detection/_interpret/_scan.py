...
def _prop_scan(
    ...
    const_inputs: list[IndexMultiSet] = [_index_sets(state_indices, v) for v in consts]
    ...
    xs_all_indices: list[IndexMultiSet] = [_index_sets(state_indices, v) for v in xs]
    ...
    num_ys = len(ys)
    ys_per_step: list[list[IndexMultiSet]] = [[] for _ in range(num_ys)]
